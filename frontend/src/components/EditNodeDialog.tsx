        ...
        <form onSubmit={handleSubmit} id="edit-node-form">
          ...
          {node && ip.trim() !== node.ip && (
            <div style={{ marginBottom: 16 }}>
              <Alert
                theme="warning"
                message={'Смена IP затрагивает WEB-прокси этой ноды: их public_addr зафиксирован при создании и участвует во внутреннем маршруте. После смены такие прокси нужно пересоздать, а A-записи их доменов перенаправить на новый адрес. Fake TLS прокси смена IP не ломает, но ссылки на них изменятся.'}
              />
            </div>
          )}
          <Alert
